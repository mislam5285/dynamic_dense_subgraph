...
for ind in xrange(0, comN):
    ...
    if runMainAlgs == 'grbi':
        ...
        #print 'before func'
        nodes_coveredGr, timeIntGr, usedBgr, Sgr, edges_coveredGr = loop_and_short_function.gamble(edgesTSGr, k, B, n, 'gr', nodes, edges)
        #print 'after func'
        #nodes_coveredGr, timeIntGr, usedBgr, S, edges_coveredGr = main.main(outpath, 'greedy', charikar_version, 'unweighted', pics, k, B, edgesTSGr, nodes, edges, initIntervals, False)
        ...
        nodes_coveredBi, timeIntBi, usedBbi, Sbi, edges_coveredBi = loop_and_short_function.gamble(edgesTSBi, k, B, n, 'bi',  nodes, edges)
        #main.main(outpath, 'binary', charikar_version, 'unweighted', pics, k, B, edgesTSBi, nodes, edges, initIntervals, False)
        ...
        tic = time.time()
        baseS, _, _, Sdy, _  = main.main(outpath, 'dynprogr', charikar_version, 'weighted', pics, k, B, edgesTS, nodes, edges, initIntervals, False)
        toc = time.time()
