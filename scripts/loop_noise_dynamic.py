...
while noise < 5.6:
    ...
    for ind in xrange(0,n):
        ...
        nodes, edges = [],[]
        edgesTS, avg_back, _ = generate.generate(k, B, 1, noise, nodesInCom-1,nodesInCom, 50, 100)
        print len(edgesTS)
        break
        #print len(edgesTS)
        #i,j = i1[ind], i2[ind]
    ...
    #print np.mean(avgBack), np.mean(precGr), np.mean(precBi), np.mean(recallGr), np.mean(recallBi)
