...
import uuid
from allutils import utils, charikar, greedy_speedup, greedy, dynprogr, plotting
...
def main(outpath, alg, charikar_version, charikar_mode, pics, k, B, edgesTS, nodes, edges, timeIntervals, baseline = False, n_disc = 101, fast = True, entire = True):   
    ...
    for maxiter in xrange(1):
        ...
        cTS = 0
        for i in edgesTS:
            if (i[1][0] in chosen) and (i[1][1] in chosen):
                cTS += 1
        print S.number_of_nodes(), S.number_of_edges(), cTS,
        
        tic = time.clock()
        if alg == 'greedy':
            if fast == True: 
                timeIntervals, edges_covered, usedB = greedy_speedup.greedy(S, edgesTS, k, timeIntervals, B)
...
if __name__ == "__main__":
    ...
    #print 'run main:'
    #for i in xrange(500,1000001,500):
    #for i in xrange(110000,999,-1000):
    for i in xrange(500,110001,500):
        print i,
        tic = time.clock()
