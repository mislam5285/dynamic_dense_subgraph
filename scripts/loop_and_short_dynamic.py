...
for ind in xrange(0,n):
    ...
    #usedBDy = B.total_seconds()/(60*60*24.0)
    
    avg_dynamic = 2.0*len(edges_coveredDy)/len(nodes_coveredDy)
    num_nodes_dy = len(nodes_coveredDy)
    num_edges_dy = len(edges_coveredDy)
    print avg_dynamic, num_nodes_dy, num_edges_dy
    toc = time.time()
    ...
    ff = open(outpath, "a")    
    ff.write(' '.join([str(avg_greedy)[:6], str(avg_binary)[:6], str(avg_dynamic)[:6],  
    str(num_nodes_gr), str(num_nodes_bi), str(num_nodes_dy),
    str(num_edges_gr), str(num_edges_bi), str(num_edges_dy),
