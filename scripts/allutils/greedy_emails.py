...
def getBestT(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a=0.0):
    i1 = 0
    iNew = -1      
    ...
    while i1 < (len(edgesTS)):        
        ...
        #print 'before', prevgain, edgesSeen
...
def run(S, edgesTS, k, timeIntervals, B, mode, a = 0.0):
    ...
    for i in xrange(0, k):
        ...
        for j in freeIntervals:
            t1, t2 = j[0], j[1]
            bestInt = getBestT(t1, nodes, edges_covered, edgesTS[t1:t2+1], rest_k, rest_B, mode)      
...
def binary(S, edgesTS, k, timeIntervals, B):
    ...
    while (ub - lb) > 1e-6:
        c += 1
        a = (lb + ub)/2       
        timeIntervals_out, edges_covered, T = run(S, edgesTS, k, timeIntervals, B, 'binary', a = 0.0)
        if T > B:
            lb = a
        if T <= B:
