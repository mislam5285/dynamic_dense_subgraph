...
def generateClique(noise):

    # noise = sys.argv[1]
    # id = sys.argv[2]
    # outpath = noise
    # outfile = noise + '_' + id + '.txt'
    ...
    d = backGrGraph.degree()    
    generated_degree = float(sum(d.values())) / len(d)
    #print "generated avg degree", float(sum(d.values())) / len(d)
    ...
    #]
    int_desc = [(0, datetime(2010, 12, 01, 00, 01, 00), datetime(2010, 12, 01, 00, 01, 20), timedelta(seconds = 1))]
    ...
    i = int_desc[0]
    num, start, end, step = i[0], i[1], i[2], i[3]
    TS += genTS3(coms[num], start, end, step, 21)
    # i = int_desc[1]
    # num, start, end, step = i[0], i[1], i[2], i[3]
