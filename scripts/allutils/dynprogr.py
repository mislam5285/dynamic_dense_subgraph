...
def dynprogr(S, edgesTS, k, timeIntervals, Budget, num):
                ...
                for j in xrange(m-1, i+1): 
                    for j_ in xrange(j+1, i+1):
                        for bs in xrange(0, b+1):
                            ...
                            #print TS[(j_-1):i,1]
                            t = np.float64(len(set(TS[(j_-1):i,1])))
                            #print t
                            cost = D[j, m-1, bs] + t
    ...
    for i in xrange(k,0,-1):
        #print end
        st = np.asscalar(Ind_[end,i, b])
        #print end, st
        #intervals.append(tuple([st-np.int64(1), end-np.int64(1)]))
        intervals.append(tuple([st-1, end-1]))
        b_new = np.asscalar(B[end,i,b])
        end_new = np.asscalar(Ind[end,i,b])
        b, end = b_new, end_new
