...
def runGreedyBS(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a, ub):
    ...
    _, lbestInt, lGain, lCost, _, lTime =  greedySubmodTrick(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, lb)
    _, ubestInt, uGain, uCost, uGraph, uTime =  greedySubmodTrick(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, ub)
    #print 'bounds:', ubestInt, uGain, lbestInt, lGain
    
    bestHScore, bestInt, bestGain, bestCost, bestGraph, bestTime =  greedySubmodTrick(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a)
    #print bestInt, len(edgesTS)
    #print bestTime, rest_B
    #if bestTime <= rest_B and bestGain > -1:
    if uGain > -1:
    ...
    while (lGain - uGain) > 1e-14 and c < 100:
        ...
        #if uTime <= rest_B and bestGain > -1:
        if bestGain > -1:
            bestIntOut = {ubestInt:(float(bestGain)/uCost, uGraph)}
    #if not bestIntOut:
    #    bestIntOut = {bestInt:(float(bestGain)/bestCost, bestGraph)}
    #print bestIntOut
    #print 'BS iterations', c, uGain, lGain, ubestInt, lbestInt, uCost, lCost, uTime, lTime, ub, lb   
    return bestIntOut
...
def getBestT_speedupBinary(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a):
    ...
    #print 'average points', float(sum(pointLen))/len(pointLen)
    print np.mean(pointLen), np.median(pointLen), np.min(pointLen), np.max(pointLen), a
    return bestInt, pointLen
    
def getBestT(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a):
    if mode == 'binary':
        bestInt, pointLen = getBestT_speedupBinary(offset, nodes, edges_covered, edgesTS, rest_k, rest_B, mode, a)
        #print bestInt.keys()
...
def run(S, edgesTS, k, timeIntervals, B, mode, a = 0.0):
    ...
    for i in xrange(0, k):
        ...
        if local_timeIntervals:
            #print 'Here', freeIntervals, new_taken            
        ...
        for j in freeIntervals:
            t1, t2 = j[0], j[1]
            if mode == 'greedy':
                bestInt = runGreedyBS(t1, nodes, edges_covered, edgesTS[t1:t2+1], rest_k, rest_B,   mode, a, ub)
            elif mode == 'binary':
                bestInt, pLen = getBestT_speedupBinary(t1, nodes, edges_covered, edgesTS[t1:t2+1], rest_k, rest_B, mode, a)      
