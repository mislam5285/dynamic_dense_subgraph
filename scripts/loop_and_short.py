...
for ind in xrange(0,n):
    ...
    #ll = min(len(timeIntBi), len(timeIntGr))
    ...
    for i in xrange(0, len(timeIntBi)):
        bii = timeIntBi[i]
        s,t = bii[0],bii[1]
        l = (edgesTS[t][0] - edgesTS[s][0]).total_seconds()/(60.0*60*24)        
        len_bi.append(l)
    ...
    for i in xrange(0, len(timeIntBi)-1):    
    ...
    for ind in xrange(0, len(timeIntBi)):      
    ...
    timeIntGr = ';'.join([i[0]+','+i[1] for i in map(str, timeIntGr)])
    timeIntBi = ';'.join([i[0]+','+i[1] for i in map(str, timeIntBi)])
    edges_coveredGr = ';'.join([i[0]+','+i[1] for i in map(str, edges_coveredGr)])
    edges_coveredBi = ';'.join([i[0]+','+i[1] for i in map(str, edges_coveredBi)])
    #initIntervals = ';'.join([i[0]+','+i[1] for i in map(str, initIntervals)])
    ...
    str(np.median(len_gr))[:6], str( np.median(len_bi))[:6],
